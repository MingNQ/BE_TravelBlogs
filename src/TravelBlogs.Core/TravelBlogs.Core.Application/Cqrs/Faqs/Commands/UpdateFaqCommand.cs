...
public class UpdateFaqCommand : FaqBaseCommand, IRequest<ResponseBase<FaqDto>>
{
    [JsonIgnore]
...
public class UpdateFaqCommandHandler(IUnitOfWork unitOfWork) : IRequestHandler<UpdateFaqCommand, ResponseBase<FaqDto>>
{
    private readonly IWriteRepository<Faq> _repo = unitOfWork.GetRepository<Faq>();

    public async Task<ResponseBase<FaqDto>> Handle(UpdateFaqCommand request, CancellationToken ct)
    {
        ...
        return new ResponseBase<FaqDto>(entity.Adapt<FaqDto>(), MessageCommon.UpdateSuccess);
    }
}
